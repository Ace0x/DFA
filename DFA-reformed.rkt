...
(define (arithmetic-lexer input-string)
  ...
  (automaton-2 (dfa-str 'start '(int var real parenth) delta-arithmetic-1) input-string))
...
(define (automaton-2 dfa input-string)
  ...
  (let loop
    ([state (dfa-str-initial-state dfa)]    ; Current state
     ...
     [result null]
     [elements '()])                         ; List of tokens found                             ; List of values
    (if (empty? chars)
      ...
      (let-values
        ...
        (loop state (rest chars) ; Update the list of tokens found
          (if token 
            (cons token result) 
            result)
          (if token
            (append (cdr chars) (elements)) 
...
(define (delta-arithmetic-1 state character)
  ...
  (case state
    ...
    (arithmetic-lexer "+(34.2)  +  +955e-24   -  -var * 23.2)")
